            ...
            foreach (var siteData in siteList)
            {
                ...
                try
                {
                    ...
                    using (HttpClient httpClient = new HttpClient())
                    {
                        ...
                        HttpResponseMessage loginPageResponse = httpClient.GetAsync(wordpressUrl).Result;
                        string loginPageHtml = loginPageResponse.Content.ReadAsStringAsync().Result;

                        string searchTag = $"<input type=\"hidden\" name=\"wp-submit\" value=\"";
                        int startIndex = loginPageHtml.IndexOf(searchTag) + searchTag.Length;
                        int endIndex = loginPageHtml.IndexOf("\"", startIndex);
                        string redirectTo = loginPageHtml.Substring(startIndex, endIndex - startIndex);

                        var formContent = new FormUrlEncodedContent(new[]
                        {
                            new KeyValuePair<string, string>("log", username),
                            new KeyValuePair<string, string>("pwd", password),
                            new KeyValuePair<string, string>("redirect_to", redirectTo),
                        ...
                        if (loginResponseHtml.Contains("profile.php"))
                        {
